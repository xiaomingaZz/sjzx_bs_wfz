...
public class JobExecuteHB  implements StatefulJob{
	private final static Log logger = LogFactory.getLog(JobExecuteHB.class);
	...
	public void GenerateXmlData(Element  element){
		...
		try {
			...
			for (String key : WebContext.fyMap.keySet()) {
				...
				completionService.submit(new CourtThreadHB(fy, tjsj, n,root));
			}
			int xssl = 0,yjsl = 0;//立案总数,结案总数
			int lasl = 0,jasl = 0;//异常过滤数量
			//等待线程完成
			for (String key : WebContext.fyMap.keySet()) {
				try {
					...
					xssl +=re.get("XSSL");
					yjsl +=re.get("YJSL");
					String fydm = String.valueOf(re.get("FYDM"));
					lasl +=re.get("LA_"+fydm+"_YC_COUNT");
					jasl +=re.get("JA_"+fydm+"_YC_COUNT");
					/*int lasjlx = re.get("LA_"+fydm+"_SJLX");
					int lasaly = re.get("LA_"+fydm+"_SALY");
					int lalaay = re.get("LA_"+fydm+"_LAAY");
					int lalaayztlx = re.get("LA_"+fydm+"_LAAYZTLX");
					int lalaayxzxwzl = re.get("LA_"+fydm+"_LAAYXZXWZL");
					int lacount = re.get("LA_"+fydm+"_COUNT");
					int layccount = re.get("LA_"+fydm+"_YC_COUNT");
					
					int jasjlx = re.get("JA_"+fydm+"_SJLX");
					int jasaly = re.get("JA_"+fydm+"_SALY");
					int jajarq = re.get("JA_"+fydm+"_JARQ");
					int jalaayztlx = re.get("JA_"+fydm+"_LAAYZTLX");
					int jalaayxzxwzl = re.get("JA_"+fydm+"_LAAYXZXWZL");
					int jalaay = re.get("JA_"+fydm+"_LAAY");
					int jajaafs = re.get("JA_"+fydm+"_JAFS");
					int jaay = re.get("JA_"+fydm+"_AY");
					int jacount = re.get("JA_"+fydm+"_COUNT");
					int jaycount = re.get("JA_"+fydm+"_YC_COUNT");*/
				} catch (Exception e) {
					logger.error("线程内部发送的异常",e);
				}
			}
			Element r = root.element("R");
			if(null == r){
				 r = root.addElement("R");
				 Element id = r.addElement("id");
				 Element drxsajsl = r.addElement("drxsajsl");
				 Element dryjajsl = r.addElement("dryjajsl");
				 Element lalist = r.addElement("lalist");
				 Element jalist = r.addElement("jalist");
				 id.setText(WebContext.allfyMap.get(WebContext.FyCode.substring(0, 2)+"0000").getDm());
				 drxsajsl.setText("0");
				 dryjajsl.setText("0");
			}
			root.addComment("当日新收案件总数:"+xssl+",异常过滤数据："+lasl);
			root.addComment("当日已结案件总数:"+yjsl+",异常过滤数据："+jasl);
		} catch (Exception e) {
			logger.error("",e);
			return;
		} finally {
