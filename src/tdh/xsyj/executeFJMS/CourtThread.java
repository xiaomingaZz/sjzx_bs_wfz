...
//				}
//				String xzxwzl = rs.getString("XZXWZL");
...
//				}
//				LogService15.insLaLog(rq,fy.getDm(),ajlxbs,ahdm18,rs.getString("AH"),rs.getString("SARQ"),rs.getString("JARQ"),rs.getString("JARQ"), rs.getString("LARQ"),
...
//						getCodeValue(CommUtil.trim(rs.getString("XZGLFW")),false),rs.getString("XZXWZL"),fy.getFydm());
			...
			logger.error("["+Thread.currentThread().getName()+"] 查询立案异常。。"+sql.toString()+","+fy.getFydm(),e);
		}finally{
...
//						rs.getString("JARQ"),getCodeValue(StringUtils.trimToEmpty(ay),true),fy.getFydm(),getCodeValue(jafs,false));
			...
			logger.error("查询结案异常。。"+sql.toString()+","+fy.getFydm(),e);
		}finally{
			DBHelper.closeResultSet(rs);
			DBHelper.closeStatement(pst);
			DBHelper.closeConn(conn);
